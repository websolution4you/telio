-- Convert role discounts from percentages to fixed EUR discounts per booked hour
-- and apply those discounts atomically to wallet bookings.
...
do $$
declare
    new_column_created boolean := false;
begin
    if not exists (
        select 1 from information_schema.columns
        where table_schema = 'public'
          and table_name = 'role_booking_policies'
          and column_name = 'discount_eur_per_hour'
    ) then
        alter table public.role_booking_policies
            add column discount_eur_per_hour numeric(6, 2);
        new_column_created := true;
    end if;

    if new_column_created or exists (
        select 1 from information_schema.columns
        where table_schema = 'public'
          and table_name = 'role_booking_policies'
        ...
        set discount_eur_per_hour = case role
            when 'trainer' then 4.00
            when 'user' then 2.00
            else 0.00
        end;
    end if;
...
alter table public.role_booking_policies
    alter column discount_eur_per_hour set default 0,
    alter column discount_eur_per_hour set not null;

alter table public.role_booking_policies
    drop constraint if exists role_booking_policies_discount_check,
    drop constraint if exists role_booking_policies_discount_eur_per_hour_check;

alter table public.role_booking_policies
    add constraint role_booking_policies_discount_eur_per_hour_check
