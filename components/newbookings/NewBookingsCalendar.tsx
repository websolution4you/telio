      ...
      <header className="relative isolate overflow-hidden border-b border-cyan-100/80 bg-gradient-to-r from-white via-cyan-50/80 to-indigo-50/80 shadow-[0_10px_35px_rgba(15,23,42,0.07)]">
        ...
        <div className="relative mx-auto flex min-h-[76px] max-w-[1500px] items-center justify-between gap-2 px-4 py-3 sm:min-h-[86px] sm:gap-4 sm:px-6 lg:px-8">
          <Link href="/" className="group flex min-w-0 items-center gap-3 sm:gap-4" aria-label="Telio domov">
            <span className="relative grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-gradient-to-br from-cyan-500 via-blue-600 to-violet-600 text-white shadow-[0_10px_24px_rgba(37,99,235,0.3),inset_0_1px_1px_rgba(255,255,255,0.5)] transition group-hover:-translate-y-0.5 sm:h-12 sm:w-12"><span className="text-lg font-extrabold" style={{ fontFamily: "var(--font-poppins), sans-serif" }}>T</span><Sparkles className="absolute -right-1 -top-1 h-3.5 w-3.5 rounded-full bg-white p-0.5 text-violet-600 shadow-sm" /></span>
          ...
          {currentUser ? <div className="flex items-center gap-1.5 sm:gap-2.5">
            <div className="flex min-w-0 items-center gap-2 rounded-2xl border border-white/90 bg-white/75 p-2 shadow-[0_8px_22px_rgba(15,23,42,0.08)] backdrop-blur-xl sm:px-3"><span className="grid h-8 w-8 shrink-0 place-items-center rounded-xl bg-gradient-to-br from-emerald-400 to-cyan-600 text-white shadow-sm"><UserRound className="h-4 w-4" /></span><span className="hidden min-w-0 sm:block"><b className="block max-w-36 truncate text-sm">{currentUser.name}</b><small className="block text-[10px] font-medium text-emerald-600">Aktívny účet</small></span></div>
          ...
          </div> : <button onClick={() => setAuth("login")} className="group flex shrink-0 items-center gap-2 rounded-2xl bg-gradient-to-r from-blue-600 via-indigo-600 to-violet-600 px-3.5 py-3 text-xs font-bold text-white shadow-[0_10px_24px_rgba(79,70,229,0.3)] transition hover:-translate-y-0.5 sm:px-5 sm:text-sm"><LogIn className="h-4 w-4" /> Prihlásiť</button>}
        </div>
        ...
        <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-xl shadow-slate-200/60">
          ...
          <div className="overflow-auto"><div className="min-w-[1050px]">
            ...
            {visibleCourts.map((court) => <div key={court.id} className="grid border-b" style={{ gridTemplateColumns: "140px 1fr" }}><div className="sticky left-0 z-20 flex min-h-20 flex-col justify-center border-r bg-white px-4"><b>{court.name}</b><small className="text-slate-500">{court.surface}</small></div><div className="relative grid" style={{ gridTemplateColumns: `repeat(${hours.length}, 1fr)` }}>{hours.map((hour) => { const label = blockedLabel(court.id, sport, hour); const past = new Date(date).setHours(hour, 0, 0, 0) < now.getTime(); return label ? <div key={hour} className="grid min-h-20 place-items-center border-r bg-amber-50 px-1 text-center text-[10px] font-bold text-amber-700">{label}</div> : past ? <div key={hour} className="min-h-20 border-r bg-slate-100" /> : <button key={hour} onClick={() => openSlot(court.id, hour)} className="group grid min-h-20 place-items-center border-r hover:bg-emerald-50"><Plus className="h-4 w-4 text-emerald-500 opacity-0 group-hover:opacity-100" /></button>; })}{isToday && currentTimePercent > 0 && <div className="pointer-events-none absolute inset-y-0 left-0 z-[1] border-r border-slate-300/80" style={{ width: `${currentTimePercent}%`, background: "repeating-linear-gradient(135deg, rgba(148,163,184,0.12) 0px, rgba(148,163,184,0.12) 5px, rgba(241,245,249,0.38) 5px, rgba(241,245,249,0.38) 10px)" }} />}
              ...
              <div className="pointer-events-none absolute inset-0 z-10">{bookings.filter((booking) => booking.courtId === court.id).map((booking) => { const own = !!currentUser && currentUser.id === booking.user_id; const canManage = own || currentUser?.role === "admin"; return <button key={booking.id} onClick={() => canManage && setDetail(booking)} className={`pointer-events-auto absolute inset-y-2 overflow-hidden rounded-lg border border-l-4 px-2.5 text-left transition duration-200 hover:z-30 hover:-translate-y-px hover:shadow-lg ${own ? "border-amber-300 border-l-amber-600 bg-amber-300 text-amber-950 shadow-[0_3px_10px_rgba(217,119,6,0.2)]" : "border-emerald-400 border-l-emerald-800 bg-emerald-500 text-white shadow-[0_3px_10px_rgba(5,150,105,0.2)]"}`} style={position(booking)} title={canManage ? "Zobraziť detail" : "Obsadené"}><span className="flex h-full min-w-0 flex-col justify-center leading-tight"><b className="block truncate text-xs font-bold tracking-normal">{formatTime(booking.start)}–{formatTime(booking.end)}</b><span className={`mt-1 block truncate text-[10px] font-medium ${own ? "text-amber-900/80" : "text-emerald-50"}`}>{own ? "Vaša rezervácia" : "Obsadené"}</span></span></button>; })}</div>{isToday && currentTimePercent > 0 && currentTimePercent < 100 && <div className="pointer-events-none absolute inset-y-0 z-20 border-l-2 border-dashed border-cyan-500 drop-shadow-sm" style={{ left: `${currentTimePercent}%` }}><span className="absolute -left-[5px] -top-1 h-2.5 w-2.5 rounded-full border-2 border-white bg-cyan-500 shadow-[0_0_0_3px_rgba(6,182,212,0.18)]" /></div>}</div></div>)}
          </div></div>
        </section>
