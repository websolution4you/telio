      ...
      <main className="mx-auto max-w-[1500px] px-4 py-8 sm:px-6 lg:py-12">
        ...
        <section className="overflow-hidden rounded-3xl border-2 border-slate-300 bg-white shadow-[0_20px_55px_rgba(15,23,42,0.10)]">
          <div className="border-b border-slate-200 p-4 sm:p-6">
            ...
            {visibleCourts.map((court) => <div key={court.id} className="grid border-b border-slate-200" style={{ gridTemplateColumns: calendarColumns }}><div className="sticky left-0 z-20 flex min-h-20 flex-col justify-center border-r-2 border-slate-300 bg-gradient-to-r from-white to-slate-50/80 px-4 shadow-[3px_0_10px_rgba(15,23,42,0.03)]"><b className="text-slate-900">{court.name}</b><small className="mt-0.5 text-slate-500">{court.surface}</small></div><div className="relative grid" style={{ gridTemplateColumns: timeColumns }}>{hours.map((hour, index) => { const label = blockedLabel(court.id, sport, hour); const past = new Date(date).setHours(hour, 0, 0, 0) < now.getTime(); const rightBorder = index === hours.length - 1 ? "" : "border-r border-slate-200"; return label ? <div key={hour} className={`grid min-h-20 cursor-not-allowed place-items-center bg-amber-50 px-1 text-center text-[10px] font-bold text-amber-700 ${rightBorder}`}>{label}</div> : past ? <div key={hour} className={`min-h-20 cursor-not-allowed bg-slate-100 ${rightBorder}`} /> : <button key={hour} onClick={() => openSlot(court.id, hour)} className={`group grid min-h-20 cursor-pointer place-items-center transition-colors duration-200 hover:bg-emerald-50/80 ${rightBorder}`}><Plus className="h-4 w-4 text-emerald-500 opacity-0 group-hover:opacity-100" /></button>; })}{isToday && currentTimePercent > 0 && <div className="pointer-events-none absolute inset-y-0 left-0 z-[1] border-r border-slate-300/80" style={{ width: `${currentTimePercent}%`, background: "repeating-linear-gradient(135deg, rgba(148,163,184,0.12) 0px, rgba(148,163,184,0.12) 5px, rgba(241,245,249,0.38) 5px, rgba(241,245,249,0.38) 10px)" }} />}<div className="pointer-events-none absolute inset-0 z-10">{bookings.filter((booking) => booking.courtId === court.id).map((booking) => { const own = !!currentUser && currentUser.id === booking.user_id; const canManage = own || currentUser?.role === "admin"; const voiceHighlight = highlightedVoiceBookings.includes(booking.id); return <button key={booking.id} onClick={() => canManage && setDetail(booking)} className={`pointer-events-auto absolute inset-y-1.5 overflow-hidden rounded-xl border px-1.5 py-1 text-center shadow-md transition duration-200 hover:scale-[1.02] ${voiceHighlight ? "voice-booking-highlight" : ""} ${canManage ? "cursor-pointer" : "cursor-not-allowed"} ${own ? "border-orange-300/90 bg-gradient-to-br from-[#D95A3F] via-[#E26A4F] to-[#C44B31] text-white shadow-[0_4px_14px_rgba(180,83,9,0.35)]" : "border-emerald-300/90 bg-gradient-to-br from-[#15803D] via-[#16A34A] to-[#14532D] text-white shadow-[0_4px_14px_rgba(22,163,74,0.35)]"}`} style={position(booking)} title={canManage ? "Zobraziť detail" : "Obsadené"}><div className="pointer-events-none absolute inset-0 opacity-25"><svg viewBox="0 0 100 100" className="h-full w-full" preserveAspectRatio="none"><rect x="5" y="5" width="90" height="90" fill="none" stroke="#FFFFFF" strokeWidth="3" /><line x1="50" y1="5" x2="50" y2="95" stroke="#FFFFFF" strokeWidth="2" strokeDasharray="6,4" /></svg></div>{voiceHighlight && <span className="voice-booking-scan" aria-hidden="true" /><b className="relative z-[1] block whitespace-normal break-words text-[clamp(8px,0.7vw,12px)] font-black leading-none drop-shadow-[0_1px_2px_rgba(0,0,0,0.85)] [overflow-wrap:anywhere]"><span className="block">{formatTime(booking.start)}</span><span className="block leading-[0.55]" aria-hidden="true">–</span><span className="block">{formatTime(booking.end)}</span></b><span className="relative z-[1] mt-0.5 block whitespace-normal break-words text-[clamp(7px,0.6vw,10px)] font-bold leading-none drop-shadow-[0_1px_2px_rgba(0,0,0,0.85)] [overflow-wrap:anywhere]">{own ? "Vaša rezervácia" : "Obsadené"}</span></button>; })}</div>{isToday && currentTimePercent > 0 && currentTimePercent < 100 && <div className="pointer-events-none absolute inset-y-0 z-20 border-l-2 border-dashed border-[#CCFF00] drop-shadow-sm" style={{ left: `${currentTimePercent}%` }} />}</div></div>)}
          </div></div>
        </section>
