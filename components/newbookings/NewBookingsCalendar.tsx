      ...
      <main className="mx-auto max-w-[1500px] px-4 py-8 sm:px-6 lg:py-12">
        ...
        <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-xl shadow-slate-200/60">
          <div className="border-b border-slate-200 p-4 sm:p-6">
            <div className="grid grid-cols-2 gap-2 lg:grid-cols-4">{sports.map((item) => <button key={item.id} onClick={() => setSport(item.id)} className={`cursor-pointer rounded-xl p-3 text-sm font-bold ${sport === item.id ? "bg-slate-950 text-white" : "bg-slate-50 text-slate-600"}`}>{item.label}</button>)}</div>
          ...
          <div className="overflow-auto"><div className="w-full" style={{ minWidth: `${calendarMinWidth}px` }}> 
            <div className="grid border-b bg-slate-50" style={{ gridTemplateColumns: calendarColumns }}><b className="sticky left-0 z-30 border-r bg-slate-50 p-4 text-xs text-slate-500">KURT</b><div className="relative grid" style={{ gridTemplateColumns: timeColumns }}>{hours.map((hour) => <b key={hour} className="border-r p-4 text-center text-xs text-slate-500">{hour}:00</b>)}{isToday && currentTimePercent > 0 && currentTimePercent < 100 && <div className="pointer-events-none absolute inset-y-0 z-20 border-l-2 border-dashed border-cyan-500" style={{ left: `${currentTimePercent}%` }}><span className="absolute left-1/2 top-1 -translate-x-1/2 whitespace-nowrap rounded-md bg-cyan-600 px-2 py-1 text-[9px] font-extrabold text-white shadow-md">{currentTimeLabel}</span></div>}</div></div>
            {visibleCourts.map((court) => <div key={court.id} className="grid border-b" style={{ gridTemplateColumns: calendarColumns }}><div className="sticky left-0 z-20 flex min-h-20 flex-col justify-center border-r bg-white px-4"><b>{court.name}</b><small className="text-slate-500">{court.surface}</small></div><div className="relative grid" style={{ gridTemplateColumns: timeColumns }}>{hours.map((hour) => { const label = blockedLabel(court.id, sport, hour); const past = new Date(date).setHours(hour, 0, 0, 0) < now.getTime(); return label ? <div key={hour} className="grid min-h-20 cursor-not-allowed place-items-center border-r bg-amber-50 px-1 text-center text-[10px] font-bold text-amber-700">{label}</div> : past ? <div key={hour} className="min-h-20 cursor-not-allowed border-r bg-slate-100" /> : <button key={hour} onClick={() => openSlot(court.id, hour)} className="group grid min-h-20 cursor-pointer place-items-center border-r hover:bg-emerald-50"><Plus className="h-4 w-4 text-emerald-500 opacity-0 group-hover:opacity-100" /></button>; })}{isToday && currentTimePercent > 0 && <div className="pointer-events-none absolute inset-y-0 left-0 z-[1] border-r border-slate-300/80" style={{ width: `${currentTimePercent}%`, background: "repeating-linear-gradient(135deg, rgba(148,163,184,0.12) 0px, rgba(148,163,184,0.12) 5px, rgba(241,245,249,0.38) 5px, rgba(241,245,249,0.38) 10px)" }} />}
