...
export default function AdminUsersDirectory() {
  ...
  const openDeleteUserModal = (user: { id: string; name: string; email: string; bookingsCount: number }) => {
  ...
  const handleDeleteUser = async (e: React.FormEvent) => {
    ...
    if (!deleteAdminPassword.trim()) {
      setDeleteError("Zadajte vaše prihlasovacie heslo.");
      return;
    }
    setDeleteLoading(true);
    setDeleteError(null);

    const res = await deleteBookingUserByAdminAction(userToDelete.id, deleteAdminPassword);
    if (res.success) {
      ...
      await loadUsers(page, query, roleFilter);
    } else {
      setDeleteError(res.error || "Používateľa sa nepodarilo zmazať.");
    }
    ...
    <div className="space-y-6 font-sans">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex flex-1 flex-col sm:flex-row items-stretch sm:items-center gap-3">
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 rounded-2xl bg-slate-950 px-4 py-2.5 text-xs sm:text-sm font-bold text-white shadow-xs transition hover:bg-slate-800 cursor-pointer shrink-0 active:scale-[0.98]"
                  ...
                  <div className="flex items-center justify-end gap-2">
                    ...
                    {currentUserId !== user.id && (
                      <button
                        ...
                        className="cursor-pointer rounded-xl border border-rose-200 bg-white p-1.5 text-rose-500 shadow-2xs transition hover:bg-rose-50 hover:text-rose-700 hover:border-rose-300"
                      >
              ...
              <div className="flex items-center gap-1.5">
                ...
                {currentUserId !== user.id && (
                  <button
                    ...
                    title="Zmazať používateľa"
                    className="rounded-lg border border-rose-200 bg-white p-1 text-rose-500 hover:bg-rose-50 hover:text-rose-700"
                  >
            ...
            <form onSubmit={handleCreateUser} className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-4">
              ...
              <div className="pt-2 flex items-center justify-end gap-2 border-t border-slate-100">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 rounded-xl bg-slate-900 px-5 py-2.5 text-xs font-bold text-white shadow-xs hover:bg-slate-800 disabled:opacity-50 cursor-pointer"
                >
                  {createUserLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : <UserPlus className="h-4 w-4 text-slate-300" />}
      ...
      {deleteModalOpen && userToDelete && (
        <div className="fixed inset-0 z-50 grid place-items-center p-3 sm:p-6" role="dialog" aria-modal="true">
          ...
          />
          <div className="relative flex max-h-[90vh] w-full max-w-md flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl animate-in fade-in zoom-in-95 duration-150">
            {/* Modal Header */}
            <div className="flex items-center justify-between border-b border-rose-100 bg-rose-50/60 p-5">
              <div className="flex items-center gap-3">
                <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-rose-100 text-rose-600 shadow-2xs">
                  <Trash2 className="h-5 w-5" />
                </div>
                <div>
                  <h3 className="text-base font-bold text-slate-950">Zmazať používateľa</h3>
                  <p className="text-xs font-semibold text-rose-600">Trvalé a nevratné vymazanie</p>
                </div>
              </div>
              <button
                type="button"
                ...
                className="rounded-xl border border-slate-200 bg-white p-2 text-slate-400 hover:bg-slate-50 hover:text-slate-700 disabled:opacity-50 cursor-pointer shadow-2xs"
              >
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            {/* Modal Form */}
            <form onSubmit={handleDeleteUser} className="space-y-4 p-5 overflow-y-auto">
              {/* User Info Card */}
              <div className="flex items-center gap-3 rounded-2xl border border-slate-200/80 bg-slate-50/80 p-3.5">
                <TennisBallAvatar name={userToDelete.name} className="h-10 w-10 rounded-xl" textSize="text-xs" />
                <div className="min-w-0 flex-1">
                  <strong className="block truncate text-sm font-bold text-slate-900">{userToDelete.name}</strong>
                  <span className="block truncate text-xs text-slate-500">{userToDelete.email}</span>
                </div>
              </div>

              {/* Booking count warning notice */}
              <div className="rounded-2xl border border-amber-200 bg-amber-50/80 p-4 text-xs text-amber-950 space-y-2">
                <div className="flex items-center gap-2 font-bold text-amber-900">
                  ...
                  <span>
                    {userToDelete.bookingsCount > 0
                      ? `Používateľ má ${userToDelete.bookingsCount} ${
                          userToDelete.bookingsCount === 1
                            ? "aktívnu/evidovanú rezerváciu"
                            : userToDelete.bookingsCount >= 2 && userToDelete.bookingsCount <= 4
                            ? "aktívne/evidované rezervácie"
                            : "aktívnych/evidovaných rezervácií"
                        }`
                      : "Používateľ nemá žiadne rezervácie"}
                  </span>
                </div>
                <p className="text-amber-800 leading-relaxed">
                  {userToDelete.bookingsCount > 0
                    ? "Zmazaním tohto používateľa sa natrvalo odstránia všetky jeho rezervácie, kreditná peňaženka a história transakcií."
                    : "Zmazaním tohto používateľa sa natrvalo odstráni jeho profil a kreditná peňaženka."}
                </p>
                <p className="font-bold text-rose-700 text-[11px] pt-1 border-t border-amber-200/60">
                  ⚠️ Táto akcia je nezvratná a dáta nemožno obnoviť.
                </p>
              </div>
              ...
              <div className="space-y-1.5">
                <label className="block text-xs font-bold text-slate-800">
                  Vaše prihlasovacie heslo administrátora <span className="text-rose-600">*</span>
                </label>
                <p className="text-[11px] text-slate-500">
                  Pre overenie vašej identity zadajte heslo, ktorým ste sa prihlásili do systému.
                </p>
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="Vaše administrátorské heslo"
                    autoFocus
                    className="w-full rounded-xl border border-slate-200 bg-slate-50 pl-9 pr-10 py-2.5 text-sm text-slate-900 outline-none transition focus:border-rose-500 focus:bg-white focus:ring-4 focus:ring-rose-100"
                  />
                  <button
                    type="button"
              ...
              {/* Error box */}
              {deleteError && (
                <div className="rounded-xl border border-rose-200 bg-rose-50 p-3 text-xs font-semibold text-rose-700 flex items-start gap-2">
              ...
              <div className="pt-2 flex items-center justify-end gap-2 border-t border-slate-100">
                <button
                  ...
                  className="rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-xs font-bold text-slate-700 hover:bg-slate-50 disabled:opacity-50 cursor-pointer"
                >
                  Zrušiť
                </button>
                <button
                  type="submit"
                  disabled={deleteLoading || !deleteAdminPassword.trim()}
                  className="flex items-center gap-1.5 rounded-xl bg-rose-600 px-5 py-2.5 text-xs font-bold text-white shadow-sm hover:bg-rose-700 disabled:opacity-50 cursor-pointer transition"
                >
                  {deleteLoading ? (
                    <>
                      <Loader2 className="h-4 w-4 animate-spin" />
                      <span>Mazanie používateľa...</span>
                    </>
                  ) : (
                    <>
                      <Trash2 className="h-4 w-4" />
                      <span>Potvrdiť a zmazať</span>
                    </>
                  )}
                </button>
              </div>
