...
import { Activity, CalendarDays, Check, Loader2, ShieldCheck, Users, Wrench, X } from "lucide-react";
import { createBookingAction, fetchAdminDashboardDataAction } from "@/app/actions/bookings";
import { fetchAdminUsersAction, updateBookingUserRoleAction } from "@/app/actions/adminUsers";
...
type AdminUser = { id: string; name: string; email: string; phone: string | null; role: BookingRole; created_at: string };

const roleLabels: Record<BookingRole, string> = { admin: "Administrátor", user: "Používateľ", trainer: "Tréner" };
...
export default function NewBookingsAdminDashboard() {
  ...
  useEffect(() => {
    let active = true;
    Promise.all([fetchAdminDashboardDataAction(), fetchAdminUsersAction()]).then(([dashboardResult, usersResult]) => {
      if (!active) return;
      ...
      if (usersResult.success) {
        ...
        setCurrentUserId(usersResult.currentUserId);
  ...
  return <div className="space-y-8">
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="mb-5 flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-indigo-100 text-indigo-700"><ShieldCheck className="h-5 w-5" /></span><div><h2 className="text-xl font-bold">Používatelia a roly</h2><p className="text-sm text-slate-500">Zmena sa prejaví pri ďalšom prihlásení používateľa.</p></div></div>
      {usersLoading ? <div className="grid min-h-32 place-items-center"><Loader2 className="h-6 w-6 animate-spin text-indigo-600" /></div> : <div className="overflow-auto"><table className="w-full min-w-[680px] text-left text-sm"><thead><tr className="border-b text-xs uppercase tracking-wider text-slate-400"><th className="pb-3">Používateľ</th><th className="pb-3">Kontakt</th><th className="pb-3">Rola</th></tr></thead><tbody>{users.map((user) => <tr key={user.id} className="border-b border-slate-100 last:border-0"><td className="py-4"><b className="block text-slate-900">{user.name}</b><small className="text-slate-400">{user.id === currentUserId ? "Tvoj účet" : `Registrovaný ${date(user.created_at)}`}</small></td><td><span className="block text-slate-700">{user.email}</span><small className="text-slate-400">{user.phone || "Bez telefónu"}</small></td><td><div className="flex items-center gap-2"><select value={user.role} disabled={user.id === currentUserId || savingUserId === user.id} onChange={(event) => void updateRole(user.id, event.target.value as BookingRole)} className="min-w-40 rounded-xl border border-slate-200 bg-white px-3 py-2 font-semibold text-slate-700 disabled:cursor-not-allowed disabled:bg-slate-100">{(Object.keys(roleLabels) as BookingRole[]).map((role) => <option key={role} value={role}>{roleLabels[role]}</option>)}</select>{savingUserId === user.id && <Loader2 className="h-4 w-4 animate-spin text-indigo-600" />}</div></td></tr>)}</tbody></table></div>}
    </section>
