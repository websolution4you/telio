            ...
            <div className="mt-4 grid gap-4 xl:grid-cols-3">{roles.map((role) => {
              ...
              return <div key={role} className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4">
                ...
                <div className="space-y-3">
                  ...
                  <label className="block text-xs font-semibold text-slate-600">Maximálne trvanie jednej rezervácie<select value={policy.maxBookingDurationMinutes} onChange={(event) => changePolicy(role, "maxBookingDurationMinutes", Number(event.target.value))} className="mt-1 w-full rounded-lg border border-slate-200 bg-white p-2.5 text-sm text-slate-900">{durationLimits.map((minutes) => <option key={minutes} value={minutes}>{minutes < 60 ? `${minutes} minút` : minutes % 60 ? `${Math.floor(minutes / 60)} h ${minutes % 60} min` : `${minutes / 60} h`}</option>)}</select></label>
                  <label className="block text-xs font-semibold text-slate-600">Zľava za každú hodinu rezervácie (€)<input type="number" min="0" max="100" step="0.01" value={policy.discountEurPerHour} onChange={(event) => changePolicy(role, "discountEurPerHour", Number(event.target.value))} className="mt-1 w-full rounded-lg border border-slate-200 bg-white p-2.5 text-sm text-slate-900" /></label>
